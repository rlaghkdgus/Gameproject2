...
public class T_PlayerData : MonoBehaviour
{
    ...
    {
        //카드주기를 여기에 넣거나
        T_TurnSys.Instance.gState.Value = GameState.WaitAction;//게임 상태를 WaitAction으로 바꾼 후
    ...
    private void SelectCardFin(PlayerState _pState)//카드 선택이 끝났을때
    {
        if (_pState == PlayerState.SelectFin)
        {
            ...
            T_CardManager.Instance.SortCards(playerCards);
            T_CardManager.Instance.ArrangeCardsBetweenMyCards(playerCards, cardLeftTransform, cardRightTransform, 1.7f);//건필군이 카드 간격을 조절하고싶을때 맨마지막상수f값을건들면됨
    ...
    IEnumerator CardClearDelay()
    {
        ...
        T_TurnSys.Instance.gState.Value = GameState.ActionEnd;  //게임상태를 ActionEnd로만듬
